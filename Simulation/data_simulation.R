...
#Nationales Ergebnis
json_data$schweiz$vorlagen$kantone[[v]]$resultat.gebietAusgezaehlt <- FALSE
json_data$schweiz$vorlagen$kantone[[v]]$resultat.jaStimmenInProzent <- runif(26,0,100)

#Gemeinden
for (k in 1:26) {
  json_data$schweiz$vorlagen$kantone[[v]]$gemeinden[[k]]$resultat.gebietAusgezaehlt <- FALSE
...
json_data_kantone$kantone$vorlagen[[k]]$vorlageBeendet <- FALSE
json_data_kantone$kantone$vorlagen[[k]]$resultat.gebietAusgezaehlt <- FALSE
json_data_kantone$kantone$vorlagen[[k]]$resultat.jaStimmenInProzent <- runif(count_vorlagen,0,100)

for (v in 1:count_vorlagen) {

json_data_kantone$kantone$vorlagen[[k]]$gemeinden[[v]]$resultat.gebietAusgezaehlt <- FALSE
